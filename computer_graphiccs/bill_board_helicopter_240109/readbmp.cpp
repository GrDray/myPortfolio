 ...
 */
int readBMP24(FILE *fp, mBITMAPINFOHEADER *info, float *imgData)
{
 ...
 * Assume the file header and info header have been read.
 */
int readBMP8(FILE *fp, mBITMAPINFOHEADER *info, float *imgData)
{
  unsigned char  buf[3*4096];  //Buffer to keep a row of image.
 ...
 * Assume the file header and info header have been read.
 */
int readBMP1(FILE *fp, mBITMAPINFOHEADER *info, float *imgData)
{
  //Not implemented yet !
  return(-1);
}
...
{
  FILE   *fp;
  mBITMAPFILEHEADER   header;
  mBITMAPINFOHEADER   info;
  int     flag=0;
  ...
  fread(&info, sizeof(mBITMAPINFOHEADER), 1, fp);
